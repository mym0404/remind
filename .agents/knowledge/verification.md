...
- `pnpm format:check`: `biome.json` 기준으로 `practices/**/*.css|json|ts|tsx` format 상태를 검사한다.
- `pnpm format`: practice workspace 파일을 루트 Biome 설정으로 포맷한다.
...
- `pnpm format:check`는 포맷만 본다. TypeScript 타입, 테스트 의도, 문서 코드의 runtime 동작은 별도 검증이 필요하다.
