...
이 저장소는 Rspress React 개념 문서, practice 해답집, CoderPad형 React practice workspace를 함께 둔다. Rspress 설정은 `rspress.config.ts`에서 시작한다.
...
Sidebar는 `rspress.config.ts`에서 만든다. 설정 파일은 `references/concepts.md`의 개념 제목과 `docs/concepts/*.mdx`의 H1, `references/sections.md`의 해답 제목과 `docs/sections/*.mdx`의 H1을 각각 맞춰 sidebar item을 만든다.
